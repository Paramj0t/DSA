...
#define freq(m, s) fo(i, sz(s1)) { m[s1[i]]++; }
...
void seive(int n, vb arr) {		

	FOR(i, 2, sqrt(n) + 1) {
		if(!arr[i]) {
			for(int j = i + i; j < sz(arr); j = j + i) {
				arr[i] = 1;
			}
		}
	}
...
void sq(int n, int p) {
	...
	if(st * st == n) root = st;
    	else if(en * en == n) root = en; // break here if it is only ans
	else root = en - 1;
...
int gcdOrHcf(int a, int b) {
	// frk nhi pdhta a chota ya b chota
	if(a == 0) return b;
...
void printM(int n) {
	...
	// ab jo ans aya h vo int mai store ho skta h loop mai ho na ho but last mai hojaig
	int answer = ans;
	pr(answer);
}
...
void lowesandHighestPrime(int n, vb arr) {
	vi lp(n);
	vi hp(n);
...
void divisors(int n) {
	vvi multiple(n, v(n, 0));

	FOR(i, 2, N) {
		for (int j = i; j < sz(arr); j = j + i) {
			multiple[i].pb(j);
		}
	}	
}
...
void solve() {
	...
	// Test case ka while chlne seh phele precomute krlo fact 1e5 + 10 tk
	...
	// precompute
	// while chala O(T) tk phele O(T * N) tk hota n^2 ab O(T + N) mtlb O(T) mtlb O(N)
	// vi(N);
	v[0] = 1;
	v[1] = 1;
	// O(N)
	...
	partial_sum(all(v), b); // new array mai dallo .. phele dallo
