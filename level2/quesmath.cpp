...
void solve() {
	...
	while(tc--) {
		...
		FOR(i, 1, sz(forward)) {
			forward[i] = gcd(a[i], a[i - 1]); // phele case a[i - 1] = 0 and it gives num itself only
		}

		ROF(i, n, 1) {
			backward[i] = gcd(a[i], a[i + 1]);
		}
	...
	partial_sum(all(arr), arr); // prefix sum
	...
	while(t--) {
		...
		fo(i, n) {
			v[s[i] - 'a'][i + 1]++; // idx + 1  idexing 1 seh
		}

		fo(i, 26) {
			// partial_sum() sab array ka
		}
		...
		while(q--) {
			...
			// if(count > 1) {}// not palidrome

			fo(i, 26) {
				int count = v[r][i] - v[l - 1][i] 
				if(count % 2 != 0) {}
			}
		}
	}
	...
	while(q--) {
		int p, q;
		re(p, q);
		int lcm = p * 1LL * q / gcd(p, q);
		int ans = multiplies_ct[p] + multiplies_ct[q];
		if(lcm < n) {
			ans -= lcm;
		}
	}
