...
bool isPrime(int n) { if(n == 0 || n == 1) return 0; int prime[n + 1]; clr(prime); FOR(i, 2, n + 1) { if(prime[i] == 0) { for(int j = i*i; j <= n; j += i) { prime[j] = 1; } } } return (prime[n] == 0) ? 1 : 0; }
...
constexpr bool iso(ll n) { return n & 1; } // odd and last bit 
