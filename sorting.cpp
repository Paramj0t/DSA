...
const int MX = 2e5+5;
const ll INF = 1e18; // not too close to LLONG_MAX 18 digits can be stored in long long 
...
string deciToBinary(int n) { string s = bitset<64> (n).to_string(); const auto loc1 = s.find("1"); if(loc1 != string::npos) return s.substr(loc1); return "0"; }
...
void insertionSort(vi a) {
    ...
    FOR(i, 1, n) {
        // for(int j = i - 1; j >= 0; --j) {
            ROF(j, i - 1, 0) {
            if(a[j] > a[j + 1]) swap(a[j], a[j + 1]);
            else break; // break yeh likhde time save hota h kyuki isse phele toh sorted h na
        }
    }
    fo(i, n) pr(a[i], " ");
}
   ...
   int i = 0, j = 0, k = l; 
    ...
    while(i < n1 && j < n2) {
        if(a[i] < b[j]) {
            ...
            arr[k] = b[j];
            // inv += n1 - i ... long long h... return last meh 
            k++;
            j++;
        }
    }
...
int partition(int arr[], int l, int r) {
    int pivot = arr[r]; // r included h r h n nhi
    int i = l - 1;
...
// count inversion sort meh ll inv = 0; bnake a[i] > b[j] inv += n1 - i; kardo and return inv kardo 3no seh return karvao

void countSort(vi arr, int n) {
    int mx = INT_MIN;

    fo(i, n) {
        mx = max(mx, arr[i]);
    }
    mx++; // max size jitna isliye ++ kara
    vi a(mx, 0);
...
void waveSort(vi arr, int n) {
    for(int i = 1; i < n; i += 2) {
        if(arr[i] > arr[i - 1]) swap(arr[i], arr[i - 1]);
        if(arr[i] > arr[i + 1] && i < n - 1) swap(arr[i], arr[i + 1]);
    }
...
int quickSelect(int arr[], int l, int r, int k) {
   if(l < r) {
    ...
    return -1; // yeh toh hoga nhi ise phele return ho chuka hoga
}
...
void solve() {
    ...
    // bubbleSort(v);
    // insertionSort(v);
    // mergeTwoSortedArr(v, a);
    // mergeSort(v);
    // int a[] = {1, 4, 1, 2};
    // quickSort(a, 0, 3);
    // fo(i, 4) pr(a[i]);

    // vi arr = {1, 3, 2, 3, 4, 1, 6, 4, 3};
    // countSort(arr, sz(arr));
    ...
    int a[] = {2, 8, 1, 3, 7, 6, 4, 5};
    pr(a[quickSelect(a, 0, 7, 2)]);
