...
bool ratinMaze(int** a, int x, int y, int n, int** solA) {
    ...
    if(isSafe(a, x, y, n)) {
        solA[x][y] = 1;
        if(ratinMaze(a, x + 1, y, n, solA)) return 1;
        if(ratinMaze(a, x, y + 1, n, solA)) return 1;
...
void nQueenProb(vector<vector<bool>> arr, int row, vb &cols, vb &nd, vb &rd, string ans) {
    ...
    fo(i, sz(arr[0])) {
        if(cols[i] == 0 && nd[i + row] == 0 && rd[row - i + sz(arr[0])] == 0) {
            arr[row][i] = 1;
            cols[i] = 1;
            nd[i + row] = 1;
            rd[row - i + sz(arr[0])] = 1;
            nQueenProb(arr, row + 1, cols, nd, rd, ans + ts(row) + " - " + ts(i) + " ");
            //backtracking ans na ai toh
            arr[row][i] = 0;
            cols[i] = 0;
            nd[i + row] = 0;
            rd[row - i + sz(arr[0])] = 0; 
...
//     }
    ...
    vb rd = {0, 0, 0, 0, 0, 0, 0};
    string ans = "";
    nQueenProb(arr, row, cols, nd, rd, ans);
}
