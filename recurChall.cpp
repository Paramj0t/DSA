...
void printd(int n) {
    if(n == 1) {
        pr(1);
        return; // yeh h break point return krne kelia
    }

    pr(n);
    printd(n - 1);
    return; // no frk daalo nhi daalo
}
...
int lastocc(int a[], int n, int i, int key) {
    ...
    int resetArray = lastocc(a, n, i + 1, key);

    if(resetArray != -1) return resetArray;
