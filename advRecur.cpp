...
//vector ka distint elements
void permutate1(vector<int> &nums, vector<vector<int>> &ans, int idx) {
    ...
    FOR(i, idx, sz(nums)) {
        ...
        permutate1(nums, ans, idx + 1);
        swap(nums[idx], nums[i]); // restoring the array for no repeation
    }
}

// stl trick
vvi permutateSTL(vi nums) {
    vvi ans;
    sort(all(nums));
    do {
        ans.pb(nums);
...
// vector ka duplicate elements allowed
void helper(vi &nums, vvi &ans, int idx) {
    if(idx == sz(nums)) {
        ans.pb(nums);
        return;
    }
...
int tilingProb(int n) { // fibonachii hi bn rha
    if(n == 0 || n == 1) return n;
...
int friendsPair(int n) {
    ...
    return friendsPair(n - 1) + friendsPair(n - 2) * (n - 1); //pairing kelia n - 1 mul kiya hai

}

int knapsackProb(int value[], int wt[], int n, int w) {

    if(n == 0 || w == 0) return 0;
    if(wt[n - 1] > w) return knapsackProb(value, wt, n - 1, w);
    return max(knapsackProb(value, wt, n - 1, w - wt[n - 1]) + value[n - 1],knapsackProb(value, wt, n - 1, w));
