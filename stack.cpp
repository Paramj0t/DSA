...
// 		void pop() {
// 			if(top == -1) {
// 				pr("Stack Empty already no ele to pop");
// 			}
...
string infixToPostfix(string s) {
	...
	fo(i, s.length()) {
		if((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z')) {
			...
			while(!st.empty() && st.top() != '(') { // empty ki condition phele coz empty h top check hogya toh error ajaiga
				res += poop(st);
			}
			if(!st.empty()) st.pop();
		} else {
...
void solve() {
	...
	// pr(postfixEvaluation("46+2/5*7+"));
	// pr(prefixEvaluation("-+7*45+20"));
	// pr(infixToPostfix("(a - b / x) * (a / k - l)"));
