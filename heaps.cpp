...
void buildHeap(int a[], int n) { // >= 0 as 0 based indexing
...
//logic 1
// void insert(int x, pqmxh &maxHeap, pqmnh &minHeap) {
...
void insert(int x, pqmxh &maxHeap, pqmnh &minHeap) {
	if(minHeap.size() != 0 and x > minHeap.top()) {
		minHeap.push(x);
	} else {
		maxHeap.push(x);
	}
...
double findMedian(pqmxh &maxHeap, pqmnh &minHeap) {
	if(maxHeap.size() == minHeap.size()) {
		return (maxHeap.top() + minHeap.top()) / 2.0; // .0 likhna mt bhulna by default int h 
...
void solve() {
	...
	// insert(10, maxHeap, minHeap);
	// // pr(findMedian(maxHeap, minHeap));
	// insert(15, maxHeap, minHeap);
	// // pr(findMedian(maxHeap, minHeap));
	// insert(21, maxHeap, minHeap);
	// // pr(findMedian(maxHeap, minHeap));
	// insert(30, maxHeap, minHeap);
	// // pr(findMedian(maxHeap, minHeap));
	// insert(18, maxHeap, minHeap);
	// // pr(findMedian(maxHeap, minHeap));
	// insert(19, maxHeap, minHeap);
	// pr(findMedian(maxHeap, minHeap));
...
int sumSub(int arr[], int n,int k) {
	pqmxh maxHeap;
	fo(i, n) maxHeap.push(arr[i]);
	int sum = 0, i = 0;
	while(!maxHeap.empty()) {
		sum += maxHeap.top();
		maxHeap.pop();
