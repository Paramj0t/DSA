...
void towerOfHanoi(int n, char src, char dest, char helper) {
    ...
    pr("Ring num ", n, " move from ", src, " to ", dest);
    prlbr();
...
string remDup(string s) {
    ...
    char ch = s[0];
    string restofs = remDup(s.substr(1));

    if(ch == restofs[0]) return restofs;
...
string movex(string s) {
    ...
    char ch = s[0];
    string restofs = movex(s.substr(1));

    if(ch == 'x') return restofs + ch;
    return ch + restofs;
}
...
void printsubascii(string s, string ans) {
   if(!s.length()) {
       prlbr(ans);
       return;
   }

   printsubascii(s.substr(1), ans);
   printsubascii(s.substr(1), ans + s[0]);
   printsubascii(s.substr(1), ts(s[0] + 0));
}

string keypadArr[] = {"", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};

void keypd(string s, string ans) {
    if(!s.length()) {
        prlbr(ans);
        return;
    }
    ...
    char ch = s[0];
    string code = keypadArr[ch - '0' - 1];
    string restofs = s.substr(1);

    fo(i, code.length()) {
        keypd(restofs, ans + code[i]);
    }
}
